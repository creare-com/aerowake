...
DATA_DIR = os.path.join('\\\\Olympus', 'Projects', '6598-Optical-Guide-II',
                        ...
                        '20170321 - Stereo tour of Creare','Tour 2 - 1600 - Copy',
                        'startup_rec_data')
...
##if height and widht calculated for each image necessary then move in back inside for loop
...
for i in range(400,file_len-1):
    LEFT_FILE='L '+'('+str(i+1)+').tif'
    RIGHT_FILE='R '+'('+str(i+1)+').tif'
    ...
    '''If original case images desired uncomment this section
    ...
    print 'Matches filtered only by distance: %s, %s' % (xy1.shape, xy2.shape)
    ...
    print 'Matches after horizontal filtering: %s, %s' % (xy1_filt.shape, xy2_filt.shape)
    ...
    points3d_filt=points3d[(points3d[:,2]>=0) & (points3d[:,2]<50000)]#,points between 0 and 50 m depth
    ...
    print 'mean and standard deviation in depth: %3.f' % np.std(dist)
...
###############VISUALIZATION OPTIONS BELOW, UNCOMMENT ANY OF THESE IF DESIRED#############
########################COULD ADD IF STATEMENT TO ACCOMPLISH THIS#########################
######If detected features in each image desired
'''
    
'''
'''
    ###### Draw first X matches, images sorted in order of their distance
    ...
    cv2.namedWindow('sorted matches',cv2.WINDOW_NORMAL)
    cv2.resizeWindow('sorted matches', 1500,600)
    cv2.imshow("sorted matches", img3)
    cv2.waitKey()
'''
######If left to right matching desired uncomment this section
'''
    view3 = sp.zeros((height, width + width), sp.uint8)
    view3[:height, :width] = left_img  
    view3[:height, width:] = right_img
    view3[:, :] = view3[:, :]  
    #view3[:, :] = view3[:, :]
    for m in xrange(len(xy1_filt)):
        # draw the keypoints
        color = tuple([sp.random.randint(0, 255) for _ in xrange(3)])
        cv2.line(view3, (int(xy1_filt[m,0]), int(xy1_filt[m,1])),(int(xy2_filt[m,0] + width), int(xy2_filt[m,1])), color)
...
'''
#####If disparity plots wanted, disparity plot for each matched features###
if thicker_line:
    left_img = plt.imread(os.path.join(DATA_DIR, LEFT_FILE))
    left_img=cv2.cvtColor(left_img,cv2.COLOR_GRAY2RGB)[:, :,0]
    print left_img.shape
    color = [(139, 0, 0), 
          (0, 100, 0),
          (0, 0, 139)]
    disparity_plot = sp.zeros((height, width), sp.uint8)
    disparity_plot[:height, :width] = left_img
...
#######matched features after filtering###########################
    view3 = sp.zeros((height, 2* width), sp.uint8)
    view3[:height, :width] = left_img  
    view3[:height, width:] = right_img
    view3[:, :] = view3[:, :]  
    view3[:, :] = view3[:, :]
